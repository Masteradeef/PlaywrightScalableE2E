// shared.vehicleDetailPage.locators.ts
...
export abstract class BaseVehicleDetailPageLocators {
  ...
  requestCARFAXHistoryReportLink = () => this.page.locator('a.scr-link.undefined').filter({ hasText: /Request report|Demander un rapport/i });
  ...
  learnMoreButton = () => this.page.locator('a.sr-button--secondary', { hasText: /^(More information?|Plus d'informations)$/i });
  stockNumber = () => this.page.locator('#basic-details-section dd.DataGrid_defaultDdStyle__3IYpG.DataGrid_fontBold__RqU01.DataGrid_lastItem__ObUNO') 
  addVehicleDataButton = () => this.page.getByText(/Add vehicle data|Ajoutez les données du véhicule/i);
  editETIButton = () => this.page.getByRole('button', { name: /Edit|Modifier/i });
  deleteETIButton = () => this.page.getByRole('button', { name: /Delete|Supprimer/i });
  alternativeOffersButton = () => this.page.getByRole('button', { name: /alternative offers|offres alternatives/i });
  // #endregion
  ...
  callTheSellerButton = (): Locator => {
    throw new Error('callTheSellerButton is not available on this device type');
  };

  closeCallTheSellerButton = (): Locator => {
    throw new Error('callTheSellerButton is not available on this device type');
  };
  ...
  //#region Legacy Trader Locators
    showAllAvailableSpecsButton = () => this.page.locator('#btn-vdp-specs-toggle');
    legacyVDPStockNumber = () => this.page.locator('#spec-value-6 > strong');
    // #endregion
}
