...
import { BaseSharedActions } from '../common/shared.common.actions';
import { MobileSharedLocators } from '../../selectors/mobile/mobile.shared.locators';
...
export class MobileSharedActions extends BaseSharedActions {
  ...
  async clickHamburgerMenu(): Promise<ActionResult> {
    try {
      await this.sharedLocators.hamburgerMenu().waitFor({ state: 'visible', timeout: 10000 });
      console.log('Hamburger menu found and visible');
  ...
  async clickMyAccountDropdown(): Promise<ActionResult> {
    try {
      await this.clickHamburgerMenu();
      await this.sharedLocators.myAccountDropdown().waitFor({ state: 'visible', timeout: 10000 });
  ...
  async clickSavedSearchesMenu(): Promise<ActionResult> {
    try {
      await this.clickMyAccountDropdown();
      await this.sharedLocators.savedSearchesLink().waitFor({ state: 'visible', timeout: 10000 });
