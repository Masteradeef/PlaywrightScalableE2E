import { BaseFavouritesPageActions } from '../common/shared.favouritesPage.actions';
import { MobileFavouritesPageLocators } from '../../selectors/mobile/mobile.favouritesPage.locators';
...
export class MobileFavouritesPageActions extends BaseFavouritesPageActions {
  ...
  async getFavoriteItemDealerInfoText(index?: number): Promise<string | null> {
    try {
      ...
      // Try to wait for visible, if not visible scroll to element and try again
      try {
