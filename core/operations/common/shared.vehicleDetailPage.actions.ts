...
import { BaseVehicleDetailPageLocators } from '../../selectors/common/shared.vehicleDetailPage.locators';
...
export abstract class BaseVehicleDetailPageActions extends BaseActions {
  ...
  async addOrRemoveFavourite(action: FavouriteAction): Promise<ActionResult> {
    try {
      ...
      if (action === FavouriteAction.Add) {
        if (currentAriaLabel === 'Add to list' || currentAriaLabel === 'Sauvegarder') {
          await this.vehicleDetailPageLocators.addRemoveFavouriteButton().click();
          await this.vehicleDetailPageLocators.addRemoveFavouriteButton().waitFor({ state: 'visible' });
        ...
        if (currentAriaLabel === 'Added to list' || currentAriaLabel === 'Sauvegardé') {
          await this.vehicleDetailPageLocators.addRemoveFavouriteButton().click();
          await this.vehicleDetailPageLocators.addRemoveFavouriteButton().waitFor({ state: 'visible' });
          await this.page.waitForTimeout(500);
        } else {
  ...
  async getCallButtonPhoneNumber(): Promise<string | null> {
    try {
      await this.vehicleDetailPageLocators.callTheSellerButton().waitFor({ state: 'visible', timeout: 5000 });
      const href = await this.vehicleDetailPageLocators.callTheSellerButton().getAttribute('href');
  ...
  async getStockNumber(): Promise<string | null> {
    try {
      await this.vehicleDetailPageLocators.stockNumber().waitFor({ state: 'visible', timeout: 5000 });
      const title = (await this.vehicleDetailPageLocators.stockNumber().innerText()).trim();
      return title;
    } catch (error) {
      console.log('OMP vehicle stock number not found or not visible');
      return null;
    }
  }

  async clickAddVehicleDataButton(): Promise<this> {
    return this.handleAsyncChainable(
      async () => {
        await this.vehicleDetailPageLocators.addVehicleDataButton().waitFor({ state: 'visible', timeout: 10000 });
        await this.vehicleDetailPageLocators.addVehicleDataButton().click({ timeout: 10000 });
      },
      'Successfully clicked Add Vehicle Data button',
      'Failed to click Add Vehicle Data button'
    );
  };

  async isAddVehicleDataButtonVisible(): Promise<boolean> {
    try {
      await this.vehicleDetailPageLocators.addVehicleDataButton().waitFor({ state: 'visible', timeout: 5000 });
      return true;
    } catch (error) {
      console.log('Add Vehicle Data button not found or not visible');
      return false;
    }
  }

  async clickEditETIButton(): Promise<this> {
    return this.handleAsyncChainable(
      async () => {
        await this.vehicleDetailPageLocators.editETIButton().waitFor({ state: 'visible', timeout: 10000 });
        await this.vehicleDetailPageLocators.editETIButton().click({ timeout: 10000 });
      },
      'Successfully clicked Edit ETI button',
      'Failed to click Edit ETI button'
    );
  };

  async clickDeleteETIButton(): Promise<this> {
    return this.handleAsyncChainable(
      async () => {
        await this.vehicleDetailPageLocators.deleteETIButton().waitFor({ state: 'visible', timeout: 10000 });
        await this.vehicleDetailPageLocators.deleteETIButton().click({ timeout: 10000 });
      },
      'Successfully clicked Delete ETI button',
      'Failed to click Delete ETI button'
    );
  };

  async clickAlternativeOffersButton(): Promise<this> {
    return this.handleAsyncChainable(
      async () => {
        await this.vehicleDetailPageLocators.alternativeOffersButton().waitFor({ state: 'visible', timeout: 10000 });
        await this.vehicleDetailPageLocators.alternativeOffersButton().click({ timeout: 10000 });
      },
      'Successfully clicked Alternative Offers button',
      'Failed to click Alternative Offers button'
    );
  };
    // #region Legacy Trader functions 
    async clickShowAllAvailableSpecsButton(): Promise<ActionResult> {
      try {
        await this.vehicleDetailPageLocators.showAllAvailableSpecsButton().click();
        return { success: true };
      } catch (error) {
        console.error('Failed to click show all available specs button:', error);
        return { success: false, message: error instanceof Error ? error.message : String(error) };
      }
    }

    async getLegacyStockNumber(): Promise<string | null> {
      try {
        await this.vehicleDetailPageLocators.legacyVDPStockNumber().waitFor({ state: 'visible', timeout: 5000 });
        return (await this.vehicleDetailPageLocators.legacyVDPStockNumber().innerText()).trim();
      } catch (error) {
        console.log('Legacy vehicle stock number not found or not visible');
        return null;
      }
    }
    // #endregion
  ...
  async clickCloseCallTheSellerButton(): Promise<ActionResult> {
    throw new Error('clickCloseCallTheSellerButton is not available on this device type');
  }
  // #endregion
}
