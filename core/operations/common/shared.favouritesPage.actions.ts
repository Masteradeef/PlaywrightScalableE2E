...
import { BaseFavouritesPageLocators } from '../../selectors/common/shared.favouritesPage.locators';
...
export abstract class BaseFavouritesPageActions extends BaseActions {
  ...
  async isListingDisplayedInFavourite(titleText: string, dealerText: string, index?: number): Promise<boolean> {
    ...
    try {
      ...
      console.log(`isListingDisplayedInFavourite - Expected Title: "${titleText}", Actual Title: "${actualTitle}"`);
      console.log(`isListingDisplayedInFavourite - Expected Dealer: "${dealerText}", Actual Dealer: "${actualDealer}"`);
      const titleMatches = titleText?.includes(actualTitle!) || false;
      const dealerMatches = dealerText?.includes(actualDealer!) || false;
      console.log(`isListingDisplayedInFavourite - Title matches: ${titleMatches}, Dealer matches: ${dealerMatches}`);
