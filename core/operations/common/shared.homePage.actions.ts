...
import { BaseHomePageLocators } from '../../selectors/common/shared.homePage.locators';
...
export abstract class BaseHomePageActions extends BaseActions {
  ...
  async getPostalCodeInputValueFromHomePage(): Promise<string | null> {
    try {
      await this.homePageLocators.postalCodeInput().waitFor({ state: 'visible', timeout: 5000 });
      const value = await this.homePageLocators.postalCodeInput().inputValue();
      return value;
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error);
      console.warn(`Postal code input not available or failed to get value: ${errorMessage}`);
      return null;
    }
  }

  async verifyLocationPersistenceOnHome(expectedValue: string, isPostalCode: boolean = true) {
  const value = await this.getPostalCodeInputValueFromHomePage();

  let extracted: string | null = null;

  if (typeof value === 'string') {
    if (isPostalCode) {
      // Normalize and extract postal code (e.g. "L6S 3J9" or "L6S3J9" -> "L6S3J9")
      const normalizedValue = value.replace(/\s+/g, '');
      extracted = normalizedValue.match(/[A-Za-z0-9]{6}/)?.[0] ?? null;
    } else {
      // Extract city name (e.g. "Toronto" from "Toronto, ON")
      extracted = value.split(',')[0].trim();
    }
  }

  console.log(
    `[Validation] ${isPostalCode ? 'Postal code' : 'City'}: got '${extracted}', expected '${expectedValue}'`
  );

  expect(extracted?.toLowerCase()).toBe(expectedValue?.toLowerCase());
}
  ...
  // #region Legacy Trader Experience actions
  async enterPostalCodeLegacy(postalCode: string): Promise<ActionResult> {
    try {
      await this.homePageLocators.postalCodeInputLegacy().waitFor({ state: 'visible', timeout: 5000 });
      await this.homePageLocators.postalCodeInputLegacy().clear();
      await this.homePageLocators.postalCodeInputLegacy().fill(postalCode);
      await this.page.waitForTimeout(3000);
      return { success: true };
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error);
      console.warn(`Legacy postal code input not available or failed: ${errorMessage}`);
      return { success: false, message: errorMessage };
    }
  }

    async clickShowMeCarsBtnLegacy(): Promise<ActionResult> {
    try {
      await this.homePageLocators.showMeCarsBtnLegacy().waitFor({ state: 'visible', timeout: 5000 });
      await this.homePageLocators.showMeCarsBtnLegacy().click();
      await this.page.waitForTimeout(3000);
      return { success: true };
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error);
      console.warn(`Legacy Show Me Cars button not available or failed: ${errorMessage}`);
      return { success: false, message: errorMessage };
    }
  }
  // #endregion
