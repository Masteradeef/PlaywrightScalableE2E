...
import { BaseSharedLocators } from '../../selectors/common/shared.shared.locators';
import { ActionResult, MarketplaceType } from '../../helpers/definitions/project.types';
...
export abstract class BaseSharedActions extends BaseActions {
  ...
  /**
   * Navigate to a specific legacy URL
   * @param url The legacy URL to navigate to (can be relative or absolute)
   * @returns Promise that resolves to this for method chaining
   */
  navigateToLegacyUrl = async (url: string): Promise<this> => {
    return this.handleAsyncChainable(
      async () => {
        ...
        while (!navigationSuccess && attempt < maxAttempts) {
          attempt++;
          try {
            console.log(`Legacy URL navigation attempt ${attempt}/${maxAttempts}`);
            await this.page.goto(url);
        ...
        // Try to accept legacy cookies if banner is present
        try {
          await this.closeLegacyCookieBanner();
        } catch (error) {
          // Legacy cookie banner might not be present, continue without error
          console.log('Legacy cookie banner not found or already accepted');
        }
        
        return true;
      },
      `Successfully navigated to legacy URL: ${url}`,
      `Failed to navigate to legacy URL: ${url}`,
      270000
    );
  };
  ...
  /**
   * Close legacy cookie banner by waiting for it to appear and clicking the close button
   * Includes polling mechanism and verification that banner is closed
   * @param timeout Timeout in milliseconds (default: 10000)
   * @param pollingInterval Polling interval in milliseconds (default: 500)
   * @returns Promise that resolves to this for method chaining
   */
  closeLegacyCookieBanner = async (timeout: number = 10000, pollingInterval: number = 250): Promise<this> => {
    return this.handleAsyncChainable(
      async () => {
        const cookieBanner = this.sharedLocators.cookieBanner();
        const closeButton = this.sharedLocators.cookieBannerLegacyCloseButton();
        ...
        while (Date.now() - startTime < timeout) {
          try {
            await cookieBanner.waitFor({ state: 'visible', timeout: pollingInterval });
            bannerDisplayed = true;
            console.log('Legacy cookie banner is displayed');
            break;
          } catch (error) {
            // Continue polling
            await this.page.waitForTimeout(pollingInterval);
        ...
        if (!bannerDisplayed) {
          console.log('Legacy cookie banner did not appear within timeout');
          return true; // Not an error, banner might not be shown
        }
        
        // Click close button to close the banner
        await closeButton.click();
        console.log('Clicked legacy cookie banner close button');
        
        // Verify banner is closed by waiting for it to be hidden
        await cookieBanner.waitFor({ state: 'hidden', timeout: 5000 });
        console.log('Legacy cookie banner is now closed');
        
        return true;
      },
      'Successfully closed legacy cookie banner',
      'Failed to close legacy cookie banner',
      timeout + 5000
    );
  };

  clickSkipIntroBtnOnWelcomeBanner = async (): Promise<this> => {
    return this.handleAsyncChainable(
      async () => {
        const skipIntroButton = this.sharedLocators.skipIntroButton();
        ...
        try {
          ...
          // Banner didn't appear, skip gracefully
          console.log('Skip intro button banner did not appear, skipping');
        }
        
        return true;
      },
      'Successfully handled skip intro button on welcome banner',
      'Failed to handle skip intro button on welcome banner'
    );
  };
  ...
  async clickDeleteButtonOnSavedSearches(): Promise<ActionResult> {
    try {
      await this.sharedLocators.savedSearchDeleteButton().first().waitFor({ state: 'visible', timeout: 10000 });
      await this.sharedLocators.savedSearchDeleteButton().first().click();
  ...
  async savedSearchSubscription(email: string): Promise<ActionResult> {
    try {
      await this.sharedLocators.savedSearchEmailInput().waitFor({ state: 'visible', timeout: 10000 });
  ...
  async clickLegacyTraderExperienceButton(): Promise<ActionResult> {
    try {
      if (await this.sharedLocators.legacyTraderExperienceButton().isVisible({ timeout: 10000 })) {
        await this.sharedLocators.legacyTraderExperienceButton().click();
        console.log('Legacy trader experience button clicked');
        return { success: true };
      } else {
        console.log('Legacy trader experience button not found or not visible');
        return { success: true };
      }
    } catch (error) {
      console.log('Legacy trader experience button not present');
      return { success: false, message: error instanceof Error ? error.message : String(error) };
    }
}

checkMarketplaceExperience = async (): Promise<MarketplaceType> => {
  const isOMP = await this.sharedLocators.contentContainerOMP().isVisible({ timeout: 5000 });
  if (isOMP) {
    console.log('[INFO] One Marketplace experience detected.');
    return MarketplaceType.OneMarketplace;
  } else {
    console.log('[INFO] Legacy Trader experience detected.');
    return MarketplaceType.LegacyTrader;
  }
}

verifySavedSearchExists = async (expectedSearchName: string, pageLoadTimeout: number): Promise<boolean> => {
      console.log(`Verifying saved search "${expectedSearchName}" exists in Saved Searches page`);
      const savedSearchItems = this.sharedLocators.saveSearchListingsTitle();
      await savedSearchItems.waitFor({ state: 'visible' , timeout: pageLoadTimeout });
      const count = await savedSearchItems.count();
      console.log(`Found ${count} saved search title elements.`);
      let found = false;
      for (let i = 0; i < count; i++) {
        const item = savedSearchItems.nth(i);
        const text = (await item.textContent())?.trim();
          console.log(`[DEBUG] .sase-title [${i}] text: "${text}"`);
        if (text && text.includes(expectedSearchName)) {
          console.log(`Matched saved search: "${text}"`);
            found = true;
            break;
        }
      }
      if (!found) {
        console.log(`Saved search with name "${expectedSearchName}" not found.`);
      }
      return found;
  }

  clickAutoTraderLogo = async (): Promise<ActionResult> => {
    try {
      // Ensure the AutoTrader logo is visible before attempting to click
      await this.sharedLocators.autoTraderLogo().waitFor({ state: 'visible' });

      // Click the logo and wait for the page to reach the desired load state in the same step
      await Promise.all([
        this.page.waitForLoadState('domcontentloaded'),
        this.sharedLocators.autoTraderLogo().click(),
      ]);

      console.log('AutoTrader logo clicked');
      return { success: true };
    } catch (error) {
      console.log('AutoTrader logo not found or not clickable');
      return { success: false, message: error instanceof Error ? error.message : String(error) };
    }
  };
// #endregion
  ...
  abstract clickSavedSearchesMenu(): Promise<ActionResult>;
  //#endregion
