// mailtrap.ts
// Bypass self-signed certificate errors (for corporate proxy environments)
  ...
  while (Date.now() - start < timeoutMs) {
    ...
    for (const message of filteredMessages) {
      try {
        ...
        // Looking for pattern: Name:<span style="...">ConsumerName</span> or Nom:<span style="...">ConsumerName</span>
        const nameMatch = htmlContent.match(/(?:Name|Nom):\s*<span[^>]*>\s*([^<]+)\s*<\/span>/i);
        if (nameMatch) {
