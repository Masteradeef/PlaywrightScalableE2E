import { ProjectType, Language, Device, Browser, ProjectContext } from '../definitions/project.types';
...
/**
 * Helper to get device and language-specific data from any object with pc/mobile and en/fr keys
...
/**
 * Helper to get language-specific data from a mobile-only object with en/fr keys
 * @param data Object containing mobile language-specific data
 * @returns The data for the current language
 */
export const getMobileTestData = <T>(data: { mobile: { en: T; fr: T } }): T => {
  const language = ProjectContextManager.getInstance().getLanguage();
  return (data as any).mobile[language];
};
